    ...
    <div className="bg-white p-4 w-full rounded-[24px] shadow-sm border border-slate-100 relative">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">
          ...
          <div className="relative" onClick={(e) => e.stopPropagation()}>
            ...
            {isMonthOpen && (
              <div className="absolute top-full left-0 mt-2 w-40 bg-white border border-slate-100 shadow-xl rounded-2xl z-50 overflow-hidden">
          ...
          <div className="relative" onClick={(e) => e.stopPropagation()}>
            ...
            {isYearOpen && (
              <div className="absolute top-full left-0 mt-2 w-28 bg-white border border-slate-100 shadow-xl rounded-2xl z-50 overflow-hidden">
      ...
      <div className="grid grid-cols-7 w-full text-center">
        ...
        {calendarDays.map((dateObj, i) => {
          const todayHL      = dateObj.currentMonth && isToday(dateObj.day);
          ...
          return (
            <div key={i} className="aspect-square flex flex-col items-center justify-center p-0.5 gap-0.5">
              <button
                onClick={() => dateObj.currentMonth && onMonthChange(new Date(year, month, dateObj.day))}
                className={`w-full h-full max-w-[32px] max-h-[32px] text-[12px] flex items-center justify-center rounded-xl transition-all
                  ...
                  ${todayHL ? "bg-black text-white shadow-lg hover:bg-black" : ""}
                `}
              >
                {dateObj.day}
              </button>
              {dayPlannings.length > 0 && (
                <div className="flex gap-0.5 justify-center">
                  {dayPlannings.slice(0, 3).map((p, idx) => (
                    <div key={idx} className="w-1 h-1 rounded-full" style={{ backgroundColor: STATUS_COLORS[p.status] ?? "#000" }} />
                  ))}
                </div>
              )}
            </div>
