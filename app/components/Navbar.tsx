...
// ─────────────────────────────────────────
// MOCK USER (remplace l'utilisateur API)
// ─────────────────────────────────────────
const mockUser = {
  firstName: "Goh",
  lastName: "Yao",
...
export default function Navbar() {
  ...
  const { firstName, lastName, role } = mockUser;
  const { unreadCount, initialized } = mockNotifications;
      ...
      <header className="fixed top-0 left-64 w-[calc(100%-16rem)] flex justify-between items-center px-6 py-4 bg-white shadow border-b border-gray-200 z-30">
        ...
        <div className="flex items-center gap-4">
          ...
          >
            ...
            <span className="text-xs font-black min-w-[28px] h-7 px-2 flex items-center justify-center rounded-full ml-1 bg-black text-white">
          ...
          <div className="relative bg-white w-[90%] max-w-lg rounded-[2.5rem] p-10 shadow-2xl flex flex-col items-center text-center space-y-8">
            ...
            <div className="space-y-3">
              ...
              <p className="text-gray-500 text-lg leading-relaxed font-medium px-4">
                Souhaitez-vous vous déconnecter ? Cette action vous déconnectera
                de votre compte. Vous pourrez vous reconnecter facilement à
                tout moment en saisissant vos identifiants.
              </p>
